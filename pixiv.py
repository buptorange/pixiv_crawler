...
def download_pic(url,i,title,name):
    headers['referer'] = refer
    response = requests.get(headers = headers,proxies = proxies,url = url,timeout = 10).content
    if len(response)<70:
        #print(response)
        url = url.replace('.jpg','.png')
        f = open(SAVE_PATH + title+'_'+name + '.png', 'wb')
        response = requests.get(headers=headers, proxies = proxies,url=url,timeout = 10).content
    else:
        f = open(SAVE_PATH+title+'_'+name+'.jpg','wb')
    f.write(response)
    print('total=',count[0])
    count[0] += 1
    f.close()
...
for ID in list:
    ...
    nameres = requests.get(headers = headers,timeout = 10,proxies = proxies,cookies = rtn_cookies,url = 'https://www.pixiv.net/member_illust.php?id='+ID).content
    n = pq(nameres.decode())
    ...
    res = requests.get(headers = headers,timeout = 10,proxies = proxies,cookies = rtn_cookies,url='https://www.pixiv.net/ajax/user/'+ID+'/profile/all')
    ...
    for epoch in multi_illu:
        ...
        res = requests.get(headers = headers,timeout = 10,proxies = proxies,cookies = rtn_cookies,url='https://www.pixiv.net/ajax/user/'+ID+'/profile/illusts',params=param)
        #print(res.url)
        doc = res.content.decode()
        a = json.loads(doc)
        get_dict = {}
        ...
        for refer,(url,title) in get_dict.items():
            t_s.append(threading.Thread(target=download_pic, args=[url,i,title,name],
                                 name='LoopThread'))
            t_s[-1].start()
        for q in t_s:
            q.join()
